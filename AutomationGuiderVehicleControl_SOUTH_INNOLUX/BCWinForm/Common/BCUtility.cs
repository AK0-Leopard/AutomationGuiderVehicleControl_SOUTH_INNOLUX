...
namespace com.mirle.ibg3k0.bc.winform.Common
{
    ...
    public class BCUtility
    {
        ...
        public static void setComboboxDataSource(ComboBox crl_comboBox, List<APOINT> data_Source)
        {
            crl_comboBox.DataSource = data_Source;
            if (crl_comboBox.AutoCompleteCustomSource.Count != 0)
            {
            ...
            var data_source_name = data_Source.Select(s => s.POINT_ID).ToArray();
            crl_comboBox.AutoCompleteCustomSource.AddRange(data_source_name);
            crl_comboBox.AutoCompleteMode = AutoCompleteMode.Suggest;
            crl_comboBox.AutoCompleteSource = AutoCompleteSource.ListItems;
            crl_comboBox.DisplayMember = "POINT_ID";
            crl_comboBox.ValueMember = "ADR_ID";
        }
