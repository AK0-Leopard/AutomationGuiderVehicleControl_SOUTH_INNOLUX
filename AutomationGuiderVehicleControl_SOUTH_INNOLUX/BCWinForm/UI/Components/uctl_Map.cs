...
namespace com.mirle.ibg3k0.bc.winform.UI.Components
{
    public partial class uctl_Map : UserControl
    {
        ...
        private void initialMapSpace()
        {
            ...
            else if (BCFUtility.isMatche(mainForm.BCApp.SCApplication.BC_ID, SCAppConstants.WorkVersion.VERSION_NAME_NORTH_INNOLUX))
            {
                ...
                defaultMaxScale = 10;
                     ...
                     BCFUtility.isMatche(mainForm.BCApp.SCApplication.BC_ID, SCAppConstants.WorkVersion.VERSION_NAME_SOUTH_INNOLUX_F4))
            {
        ...
        private bool readAddressPortDatas()
        {
            ...
            try
            {
                ...
                foreach (AADDRESS adr in enumerAdr)
                {
                    ...
                    APOINT point = mainForm.BCApp.SCApplication.MapBLL.getPointByAdrID(adr.ADR_ID);
                    if (point != null)
                    {
        ...
        public uctlAddress[] GetUctlAddresses()
        {
            return m_objItemAddr;
        }
        ...
        private void refreshPortIcon()
        {
            try
            {
                if (m_objItemPortNew == null || m_objItemPortNew.Count() == 0)
                {
                    return;
                }
                ALINE line = mainForm.BCApp.SCApplication.getEQObjCacheManager().getLine();
                List<ACMD_MCS> cmd_mcss = line.CurrentExcuteACMD_MCS;
                if (cmd_mcss == null || cmd_mcss.Count() == 0)
                {
                    foreach (var obj in m_objItemPortNew)
                    {
                        obj.SetCurrentCSTID("");
                    }
                    return;
                ...
                foreach (var obj in m_objItemPortNew)
                {
                    string short_port_name = obj.p_PortName.Substring(0, 8);
                    var mcs_cmd = cmd_mcss.Where(cmd => cmd.HOSTSOURCE.Contains(short_port_name) &&
                    ...
                    {
                        obj.SetCurrentCSTID(mcs_cmd.CARRIER_ID);
                    }
        ...
        public void reAdjustTheLayerOrder()
        {
            if (m_objItemAddr != null)
            {
                for (int ii = 0; ii < m_objItemAddr.Length; ii++)
        ...
        private bool BidingRailPointAndGroupForRail()
        {
            ...
            foreach (AGROUPRAILS group_rail in listGroup)
            {
                ...
                if (!m_DicSectionGroupRails.ContainsKey(section_id))
                {
                    ...
                    m_DicSectionGroupRails.Add(group_rail.SECTION_ID.Trim(), new GroupRails(group_rail.SECTION_ID, ASEC_ObJ.SEC_DIS,
                                                                                            ASEC_ObJ.SEC_SPD.HasValue ? ASEC_ObJ.SEC_SPD.Value : 0,
                                                                                            group_rail.DIR,
