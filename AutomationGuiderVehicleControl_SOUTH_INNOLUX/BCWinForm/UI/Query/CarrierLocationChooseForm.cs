...
namespace com.mirle.ibg3k0.bc.winform.UI
{
    public partial class CarrierLocationChooseForm : Form
    {
        ...
        public CarrierLocationChooseForm(SCApplication _scApp, ACMD_MCS transfer)
        {
            InitializeComponent();
            scApp = _scApp;
        ...
        private void initialForceFinishStatus()
        {
            cmd_force_finish_status.DataSource = Enum.GetValues(typeof(CompleteStatus)).Cast<CompleteStatus>()
                                      .Where(e => e == CompleteStatus.CmpStatusForceFinishByOp
                                                  ).ToList();
        }
        ...
        private void initialVhLocationCmd()
        {
            ...
            lstVh.AddRange(scApp.VehicleBLL.cache.loadAllVh().Select(vh => vh.Real_ID).ToList());
            Common.BCUtility.setComboboxDataSource(cmb_vhIDs, lstVh.ToArray());

        }

        private void initialCarrierInfo(ACMD_MCS transfer)
        {
            ...
            string cmd_id = SCUtility.Trim(transfer.CMD_ID, true);
