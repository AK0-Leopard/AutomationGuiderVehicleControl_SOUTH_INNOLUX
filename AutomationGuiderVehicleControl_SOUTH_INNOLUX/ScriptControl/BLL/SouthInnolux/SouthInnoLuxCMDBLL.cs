...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class SouthInnoLuxCMDBLL : CMDBLL
    {
            ...
            if (System.Threading.Interlocked.Exchange(ref syncTranCmdPoint, 1) == 0)
            {
                ...
                try
                {
                    ...
                    if (DebugParameter.CanAutoRandomGeneratesCommand || (scApp.getEQObjCacheManager().getLine().SCStats == ALINE.TSCState.AUTO && scApp.getEQObjCacheManager().getLine().MCSCommandAutoAssign))
                    {
                        ...
                        if (idle_vh_count > 0)
                        {
                            //List<ACMD_MCS> queue_acmd_mcss = scApp.CMDBLL.loadMCS_Command_Queue();
                            List<ACMD_MCS> queue_acmd_mcss = unfinish_mcs_cmd.Where(cmd => cmd.TRANSFERSTATE == E_TRAN_STATUS.Queue).ToList();
                            List<ACMD_MCS> port_priority_max_command = null;
                            if (queue_acmd_mcss != null && queue_acmd_mcss.Count > 0)
                            {

                                port_priority_max_command = new List<ACMD_MCS>();
                                foreach (ACMD_MCS cmd in queue_acmd_mcss)
                                {
                                ...
                                {
                                    timeout_command = queue_acmd_mcss.
                                ...
                                {
                                    search_nearest_mcs_cmd = queue_acmd_mcss;
                                }
                                ...
                                foreach (ACMD_MCS first_waitting_excute_mcs_cmd in queue_acmd_mcss)
                                {
                                    string check_result = "";
                                    //ACMD_MCS excute_cmd = ACMD_MCSs[0];
                            ...
                            foreach (ACMD_MCS waitting_excute_mcs_cmd in queue_acmd_mcss)
                            {
                                int AccumulateTime_minute = 1;
                                //int AccumulateTime_minute = 2;
        ...
        public override bool IsCMD_MCS_RetryOverTimes(string cmdID)
        {
            ...
            {
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(SouthInnoLuxCMDBLL), Device: string.Empty,
                   Data: $"Cmd id:{cmdID} check retry count ,current count:{cmd_mcs.RetryTimes},max retry count:{SystemParameter.AGVCLoadingInterlockErrorMaxRetryCount}");
