...
namespace com.mirle.ibg3k0.sc.BLL
{
    ...
    /// Class EventBLL.
    /// </summary>
    public class NorthInnoLuxEventBLL: EventBLL
    {
        /// <summary>
        /// The sc application
        /// </summary>
        ...
        /// <param name="isEnable">The is enable.</param>
        public override bool enableEventReport(string ceid, Boolean isEnable)
        {
            bool isSuccess = true;
            DBConnection_EF conn = null;
            try
        ...
        public override List<AEVENTRPTCOND> LoadAllCEID()
        {
            DBConnection_EF conn = null;
            List<AEVENTRPTCOND> condList = null;
            try
            {
                conn = DBConnection_EF.GetContext();
                conn.BeginTransaction();
                condList = eventRptCondDao.LoadRptCond(conn);
                conn.Commit();
        ...
        public override List<AEVENTRPTCOND> LoadEnabledCEID()
        {
            DBConnection_EF conn = null;
            List<AEVENTRPTCOND> condList = null;
            try
            {
                conn = DBConnection_EF.GetContext();
                conn.BeginTransaction();
                condList = eventRptCondDao.LoadRptCond(conn, SCAppConstants.YES_FLAG);
                conn.Commit();
