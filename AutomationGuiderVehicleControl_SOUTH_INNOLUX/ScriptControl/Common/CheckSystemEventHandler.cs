        ...
        private void AMS_EVENT_HANDLER(object sender, StanMsgHandlerArgs e)
        {
            ...
            switch (illegal_code)
            {
                ...
                case SCAppConstants.REDIS_EVENT_CODE_NOTICE_THE_VH_NEEDS_TO_CHANGE_THE_PATH:
                    ...
                    app.VehicleService.VhicleChangeThePath(vh_id);
                    break;
                default:
                    break;
            }
        }
        private void RedisEventHandler(RedisChannel channel, RedisValue value)
        {
            RedisValue illegalInfoValue = redisCacheManager.ListLeftPopAsync(channel);
            string sillegalInfoValue = (string)illegalInfoValue;
            string[] eventInfo = sillegalInfoValue.Split(',');
            string vh_id = eventInfo[0];
            string illegal_code = eventInfo[1];
            string time = eventInfo[2];
