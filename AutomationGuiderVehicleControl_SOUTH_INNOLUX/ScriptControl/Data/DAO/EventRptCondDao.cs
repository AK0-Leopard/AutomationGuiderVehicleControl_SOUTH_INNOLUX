...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    ...
    public class EventRptCondDao : DaoBase
    {
        ...
        public List<AEVENTRPTCOND> LoadRptCond(DBConnection_EF conn)
        {
        ...
        public List<AEVENTRPTCOND> LoadRptCond(DBConnection_EF conn, string flag)
        {
            List<AEVENTRPTCOND> condList = null;
            try
            {
                var query = from rptid in conn.AEVENTRPTCOND
                            where rptid.ENABLE_FLG == flag.Trim()
                            select rptid;
                condList = query.ToList();
