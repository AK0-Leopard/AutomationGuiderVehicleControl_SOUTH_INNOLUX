...
namespace ObjectConverter_AGVC_NORTH_INNOLUX.BLL
{
	public class AlarmBLL  : IAlarmBLL
	{
		...
		private AlarmDao alarm_DAO = null;
			 ...
			 bool includeClear = false, string eqptID = null, string alarmCode = null)
		{
			...
			DateTime time = DateTime.ParseExact("20200218145512", "yyyyMMddHHmmss", null, DateTimeStyles.AllowWhiteSpaces);
			try
			{
				using (DBConnection_EF con = DBConnection_EF.GetUContext())
				{
					//alarms = alarm_DAO.loadAlarmByConditions(con, startDatetime, endDatetime, includeClear, eqptID, alarmCode);
				}
				return alarms;
			}
			catch (Exception ex)
