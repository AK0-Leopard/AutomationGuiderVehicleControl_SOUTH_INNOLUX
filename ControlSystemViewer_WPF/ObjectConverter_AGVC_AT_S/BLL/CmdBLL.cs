...
using ViewerObject;
using VTRANSFER = com.mirle.ibg3k0.sc.VTRANSFER;
...
{ 
    public class CmdBLL : ICmdBLL
    {
        ...
        private List<ACMD> loadUnfinishCmds()
        {
            List<ACMD> cmds = new List<ACMD>();
        ...
        private ACMD getCmdByTransferID(string transfer_id)
        {
            ...
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                var query = from cmd in con.ACMD.AsNoTracking()
                            where cmd.TRANSFER_ID.Trim() == transfer_id
                            select cmd;
        ...
        private ACMD getExecuteCmdByVhID(string vh_id)
        {
        ...
        private List<HCMD> getHCmdByConditions(DateTime startDatetime, DateTime endDatetime, string cst_id = null, string vh_id = null)
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                if (!string.IsNullOrWhiteSpace(connectionString))
                ...
                var query = from cmd in con.HCMD.AsNoTracking()
        ...
        private List<VTRANSFER> loadUnfinishedTransfers()
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                if (!string.IsNullOrWhiteSpace(connectionString))
        ...
        public List<ViewerObject.VTRANSFER> GetHTransferByConditions(DateTime startDatetime, DateTime endDatetime)
        {
            ...
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                var query = from tsf in con.HTRANSFER.AsNoTracking()
                            join cmd in con.HCMD.AsNoTracking() on tsf.ID equals cmd.TRANSFER_ID
                            ...
                            select new ViewerObject.VTRANSFER
                            {
                                ...
                                TRANSFER_STATUS = tsf.TRANSFERSTATE == E_TRAN_STATUS.Queue ? ViewerObject.VTRANSFER_Def.TransferStatus.Queue :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.PreInitial ? ViewerObject.VTRANSFER_Def.TransferStatus.PreInitial :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.Initial ? ViewerObject.VTRANSFER_Def.TransferStatus.Initial :
                                                  ...
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.Canceled ? ViewerObject.VTRANSFER_Def.TransferStatus.Canceled :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.Aborted ? ViewerObject.VTRANSFER_Def.TransferStatus.Aborted :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.Complete ? ViewerObject.VTRANSFER_Def.TransferStatus.Complete :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.RouteChanging ? ViewerObject.VTRANSFER_Def.TransferStatus.RouteChanging :
                                                  tsf.TRANSFERSTATE == E_TRAN_STATUS.Reject ? ViewerObject.VTRANSFER_Def.TransferStatus.Reject :
        ...
        private List<VCMD_OHTC> gethcmdohtcbyconditions(DateTime startDatetime, DateTime endDatetime, string cst_id = null, string vh_id = null)
        {
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var joinquery = from VHCMD_MCSi in con.HTRANSFER.AsNoTracking()
                                    join sbMCS in con.HCMD.AsNoTracking() on new
                                    {
                                        CMDID = VHCMD_MCSi.EXCUTE_CMD_ID,
                                        CARRIER_ID = VHCMD_MCSi.CARRIER_ID
                                    } equals new
                                    {
                                        CMDID = sbMCS.ID,
                                        CARRIER_ID = sbMCS.CARRIER_ID
                                    } into VHCMD_MCS
                                    from  sbOHTC in VHCMD_MCS.DefaultIfEmpty()
                                    select new
                                    {
                                        CMD_ID_MCS= VHCMD_MCSi.ID,
                                        VH_ID= sbOHTC.VH_ID,
                                        CARRIER_ID = VHCMD_MCSi.CARRIER_ID,
                                        HOSTSOURCE = VHCMD_MCSi.HOSTSOURCE,
                                        HOSTDESTINATION= VHCMD_MCSi.HOSTDESTINATION,
                                        MCS_CMD_INSERTTIME = VHCMD_MCSi.CMD_INSER_TIME,
                                        MCS_CMD_START_TIME = VHCMD_MCSi.CMD_START_TIME,
                                        MCS_CMD_FINISH_TIME = VHCMD_MCSi.CMD_FINISH_TIME,
                                        OHTC_CMD_TYPE = sbOHTC.CMD_TYPE,
                                        OHTC_CMD_STATUS= sbOHTC.CMD_STATUS,
                                        MCS_COMMANDSTATE= VHCMD_MCSi.COMMANDSTATE,
                                        OHTC_CMD_START_TIME= sbOHTC.CMD_START_TIME,
                                        OHTC_CMD_END_TIME=sbOHTC.CMD_END_TIME,
                                        OHTC_COMPLETE_STATUS=sbOHTC.COMPLETE_STATUS
                                    };


                    var query = from sbTable in joinquery.AsNoTracking()
                                where (sbTable.MCS_CMD_START_TIME >= startDatetime && sbTable.MCS_CMD_START_TIME < endDatetime)
                                &&
                                 (sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.Unload ||         //normal end
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.Loadunload ||
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.InterlockError ||  //interlock
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.VehicleAbort ||    //vehicleabort 
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.IdreadFailed       //barcoderead
                                                   ) &&
                    ...
                    foreach (var item in query?.ToList())
                    {
                        ...
                        VCMD_OHTC oVCMD_OHTC = new VCMD_OHTC()
                        {
                            ...
                            MCS_CMD_INSERTTIME = item.MCS_CMD_INSERTTIME,
                            MCS_CMD_START_TIME = item.MCS_CMD_START_TIME,
                            MCS_CMD_FINISH_TIME = item.MCS_CMD_FINISH_TIME,
                            OHTC_CMD_TYPE = defCvt.GetCmdType(item.OHTC_CMD_TYPE),
                            OHTC_CMD_STATUS = defCvt.GetCmdStatus(item.OHTC_CMD_STATUS),
                            MCS_COMMANDSTATE = defCvt.GetCommandStateInt(item.MCS_COMMANDSTATE),
                            OHTC_CMD_START_TIME = item.OHTC_CMD_START_TIME,
                            OHTC_CMD_END_TIME = item.OHTC_CMD_END_TIME,
                            OHTC_COMPLETE_STATUS = defCvt.GetOHTCCompleteStatus(item.OHTC_COMPLETE_STATUS)
                        };
        ...
        private List<ViewerObject.VMCBF> loadCountMTTRHCmd(DateTime StartTime, DateTime FinishTime)
        {
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var subquery = from sbTable in joinquery
                                   where (sbTable.MCS_CMD_START_TIME < FinishTime && sbTable.MCS_CMD_START_TIME >= StartTime)
                                         &&
                                         (sbTable.MCS_COMMANDSTATE == 128 || sbTable.MCS_COMMANDSTATE == 256 || sbTable.MCS_COMMANDSTATE == 512 || sbTable.MCS_COMMANDSTATE == 1024 || sbTable.MCS_COMMANDSTATE == 2048) &&
                                         sbTable.VH_ID != null
                                   select new
                                   {
                                       VH_ID = sbTable.VH_ID,
                                       ...
                                       OHTC_CMD_STATUS = sbTable.OHTC_CMD_STATUS
                                   };
                    ...
                    var query = from cmd in subquery
                                ...
                                select new
                                {
                                    VH_ID = g.Key.VH_ID,
                                    CMDCount = g.Count(),
                                    EQPTAbortErrorCount = g.Sum(i => (i.OHTC_CMD_STATUS == E_CMD_STATUS.AbnormalEndByOHT) ? (Int32)1 : 0),
                                };
            ...
            */
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var queryNumber = from sbTable in con.HTRANSFER.AsNoTracking()
                                      join sbOHTC in con.HCMD.AsNoTracking() on sbTable.EXCUTE_CMD_ID equals sbOHTC.ID
                                      where (sbTable.CMD_INSER_TIME < FinishTime && sbTable.CMD_INSER_TIME >= StartTime)
                                              &&
                                             (sbTable.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish || sbTable.COMMANDSTATE == (int)E_MCSCommandState.DoubleStorage || sbTable.COMMANDSTATE == (int)E_MCSCommandState.EmptyRetrieval || sbTable.COMMANDSTATE == (int)E_MCSCommandState.InterLockError || sbTable.COMMANDSTATE == (int)E_MCSCommandState.VehicleAbort) &&
                                             sbOHTC.VH_ID != null
                                      group sbTable by new { VH_ID = sbOHTC.VH_ID } into g
                                      select new
                                      {
                                          VHIDNum = g.Key
                                      };

                    VHNumbers = queryNumber?.ToList().Count() ?? 0;


                    var joinquery = from sbOHTC in con.HTRANSFER.AsNoTracking()
                                    join sbMCS in con.HCMD.AsNoTracking() on new
                                    {
                                        CMDID = sbOHTC.EXCUTE_CMD_ID,
                                        CARRIER_ID = sbOHTC.CARRIER_ID
                                    } equals new
                                    {
                                        CMDID = sbMCS.ID,
                                        CARRIER_ID = sbMCS.CARRIER_ID
                                    } into VHCMD_MCS
                                    from VHCMD_MCSi in VHCMD_MCS.DefaultIfEmpty()
                                    select new
                                    {
                                        MCS_CMD_START_TIME = VHCMD_MCSi.CMD_START_TIME,
                                        MCS_COMMANDSTATE = sbOHTC.COMMANDSTATE,
                                        VH_ID = sbOHTC.ID,
                                        OHTC_CMD_TYPE = VHCMD_MCSi.CMD_TYPE,
                                        OHTC_CMD_STATUS = VHCMD_MCSi.CMD_STATUS,
                                        CMD_INSER_TIME = VHCMD_MCSi.CMD_INSER_TIME,
                                        COMMANDSTATE = sbOHTC.COMMANDSTATE,
                                        HOSTSOURCE = sbOHTC.HOSTSOURCE,
                                        HOSTDESTINATION = sbOHTC.HOSTDESTINATION,
                                        CMD_FINISH_TIME = VHCMD_MCSi.CMD_INSER_TIME
                                    };


                    var query = from cmd in (from sbTable in joinquery
                                             where (sbTable.CMD_INSER_TIME < FinishTime && sbTable.CMD_INSER_TIME >= StartTime)
                                                     &&
                                                   (sbTable.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish || sbTable.COMMANDSTATE == (int)E_MCSCommandState.DoubleStorage || sbTable.COMMANDSTATE == (int)E_MCSCommandState.EmptyRetrieval || sbTable.COMMANDSTATE == (int)E_MCSCommandState.InterLockError || sbTable.COMMANDSTATE == (int)E_MCSCommandState.VehicleAbort) &&
                                                    sbTable.VH_ID != null
                                             select new
                                             {
                                                 HOSTSOURCE = sbTable.HOSTSOURCE,
                                                 HOSTDESTINATION = sbTable.HOSTDESTINATION,
                                                 CMD_START_TIME = sbTable.MCS_CMD_START_TIME,
                                                 COMMANDSTATE = sbTable.COMMANDSTATE,
                                                 CMD_INSER_TIME = sbTable.CMD_INSER_TIME,
                                                 CMD_FINISH_TIME = sbTable.CMD_FINISH_TIME,
                                                 OHTC_CMD_STAUTS = sbTable.OHTC_CMD_STATUS
                                             }
                                            )
                                ...
                                select new
                                {
                                    ...
                                    EntryCount = g.Sum(i => (i.CMD_START_TIME < FinishTime && i.CMD_START_TIME >= StartTime) ? (Int32)1 : 0),
                                    CompletedCount = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (Int32)1 : 0),
                                    LoadCount = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.EmptyRetrieval ? (Int32)1 : 0),
                                    UnLoadCount = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.DoubleStorage ? (Int32)1 : 0),
                                    InterLockCount = g.Sum(i => (i.COMMANDSTATE == (int)E_MCSCommandState.InterLockError) ? (Int32)1 : 0),
                                    EQPTAbortErrorCount = g.Sum(i => (i.OHTC_CMD_STAUTS == E_CMD_STATUS.AbnormalEndByOHT) ? (Int32)1 : 0),
                                    LeadTime128 = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_INSER_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    TransExecTime = g.Sum(i => (DbFunctions.DiffMinutes(i.CMD_START_TIME, i.CMD_FINISH_TIME))) ?? 0,
                                    TransExecTime128 = g.Sum(i => i.COMMANDSTATE == 128 ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    MaxTransExecTime = (int)(g.Max(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0),
                                    MinTransExecTime = (int?)(g.Min(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : null) ?? null),

                                    LoadingInterLock = g.Sum(i => ((i.COMMANDSTATE == (int)E_MCSCommandState.InterLockError) && ((i.COMMANDSTATE & 4) == 4) && ((i.COMMANDSTATE & 32) != 32)) ? (int)1 : 0),
                                    UnLoadingInterLock = g.Sum(i => ((i.COMMANDSTATE == (int)E_MCSCommandState.InterLockError) && ((i.COMMANDSTATE & 32) == 32)) ? (int)1 : 0)
                    ...
                    foreach (var item in query?.ToList())
                    {
                        transExecRates.Add(new ViewerObject.REPORT.VTransExecRate(
                                            ...
                                            _MCSTotalCount: item.MCSTotalCount, _EntryCount: item.EntryCount, _CompletedCount: item.CompletedCount, _EmptyRetrievalErrorCount: item.LoadCount, _DoubleStorageErrorCount: item.UnLoadCount, _InterLockCount: item.InterLockCount, _EQPTAbortErrorCount: item.EQPTAbortErrorCount,
                                            _BarcodeReadFailCount: 0, _ScanCount: 0,
                                             _LoadingInterLock: item.LoadingInterLock, _UnLoadingInterLock: item.UnLoadingInterLock,
                                            _AvgLeadTime: (item.CompletedCount != 0) ? Math.Round((double)(item.LeadTime128 / item.CompletedCount), 2) : 0,
                                            _CycleTime: item.TransExecTime, _AvgCycleTime: (item.CompletedCount != 0) ? Math.Round((double)(item.TransExecTime128 / item.CompletedCount), 2) : 0, _MaxCycleTime: item.MaxTransExecTime, _MinCycleTime: item.MinTransExecTime,
                                            StartTime: StartTime, EndTime: FinishTime, _TransTime128: item.LeadTime128, _TransExecTime128: item.TransExecTime128
                                            )
                                          );
                    }
                }
        ...
        private List<ViewerObject.REPORT.VTransExecRateByVHID> loadtransexecrategroupbyeqpt(DateTime StartTime, DateTime FinishTime)
        {
            ...
            MIN(case when  SubTable.COMMANDSTATE=128 then DATEDIFF(SECOND,SubTable.CMD_INSER_TIME,SubTable.CMD_FINISH_TIME) else 99999 end ) as MinLeadTime,
            ...
            MIN(case when  SubTable.COMMANDSTATE=128 then DATEDIFF(SECOND,SubTable.CMD_START_TIME,SubTable.CMD_FINISH_TIME) else 99999 end ) as MinCycleTime,
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var joinquery = from sbOHTC in con.HTRANSFER.AsNoTracking()
                                    ...
                                    select new
                                    {
                                        ...
                                        HOSTDESTINATION = sbOHTC.HOSTDESTINATION,
                                        CMD_FINISH_TIME = VHCMD_MCSi.CMD_INSER_TIME,
                                        CMD_TYPE = VHCMD_MCSi.CMD_TYPE
                                    };

                    var query = from cmd in joinquery
                                group cmd by new { VH_ID = cmd.VH_ID } into g
                                select new
                                {
                                    VH_ID = g.Key,
                                    ...
                                    EQPTAbortErrorCount = g.Sum(i => (i.OHTC_CMD_STATUS == E_CMD_STATUS.AbnormalEndByOHT) ? (Int32)1 : 0),
                                    LeadTime128 = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_INSER_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    LeadTime128_5 = g.Sum(i => (i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish && i.CMD_TYPE == E_CMD_TYPE.LoadUnload) ? (DbFunctions.DiffSeconds(i.CMD_INSER_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,

                                    TransExecTime = g.Sum(i => (DbFunctions.DiffMinutes(i.MCS_CMD_START_TIME, i.CMD_FINISH_TIME))) ?? 0,
                                    TransExecTime128 = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    TransExecTime128_5 = g.Sum(i => (i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish && i.CMD_TYPE == E_CMD_TYPE.LoadUnload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    MaxTransExecTime = (int)(g.Max(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0),
                                    MinTransExecTime = (int?)(g.Min(i => (i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish && i.CMD_TYPE == E_CMD_TYPE.LoadUnload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.CMD_FINISH_TIME)) : null) ?? null),
            ...
            */
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var queryNumber = from sbTable in joinquery
                                      where (sbTable.CMD_INSER_TIME < FinishTime && sbTable.CMD_INSER_TIME >= StartTime)
                                              &&
                                             (sbTable.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish || sbTable.COMMANDSTATE == (int)E_MCSCommandState.DoubleStorage || sbTable.COMMANDSTATE == (int)E_MCSCommandState.EmptyRetrieval || sbTable.COMMANDSTATE == (int)E_MCSCommandState.InterLockError || sbTable.COMMANDSTATE == (int)E_MCSCommandState.VehicleAbort) &&
                                             sbTable.VH_ID != null
                    ...
                    var query = from cmd in (from sbTable in joinquery
                                ...
                                group cmd by new
                                {
                                    dyear = SqlFunctions.DatePart("year", cmd.CMD_FINISH_TIME),
                                    dmonth = SqlFunctions.DatePart("month", cmd.CMD_FINISH_TIME),
                                    dday = SqlFunctions.DatePart("day", cmd.CMD_FINISH_TIME),
                                    dhour = SqlFunctions.DatePart("hour", cmd.CMD_FINISH_TIME),
                                ...
                                select new
                                {
                                    ...
                                    TransExecTime = g.Sum(i => (DbFunctions.DiffMinutes(i.CMD_START_TIME, i.CMD_FINISH_TIME))) ?? 0,
                                    TransExecTime128 = g.Sum(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0,
                                    MaxTransExecTime = (int)(g.Max(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : 0) ?? 0),
                                    MinTransExecTime = (int?)(g.Min(i => i.COMMANDSTATE == (int)E_MCSCommandState.CommandFinish ? (DbFunctions.DiffSeconds(i.CMD_START_TIME, i.CMD_FINISH_TIME)) : null) ?? null),
                    ...
                    foreach (var item in query?.ToList())
                    {
                        transExecRateDetails.Add(new ViewerObject.REPORT.VTransExecRateDetail(
                                             ...
                                             _AvgLeadTime: (item.CompletedCount != 0) ? Math.Round((double)(item.LeadTime128 / item.CompletedCount), 2) : 0,
                                             _CycleTime: item.TransExecTime, _AvgCycleTime: (item.CompletedCount != 0) ? Math.Round((double)(item.TransExecTime128 / item.CompletedCount), 2) : 0, _MaxCycleTime: item.MaxTransExecTime, _MinCycleTime: item.MinTransExecTime,
                                             StartTime: StartTime, EndTime: FinishTime, _TransTime128: item.LeadTime128, _TransExecTime128: item.TransExecTime128, _VCount: VHNumbers
                                             )
                                           );
                    }
                }
            ...
            */
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = (from sbTable in con.HTRANSFER.AsNoTracking()
                                 orderby sbTable.CMD_INSER_TIME ascending
                                 select sbTable.CMD_INSER_TIME).Take(1);
                    ...
                    var query1 = (from sbTable in con.HTRANSFER.AsNoTracking()
                                  orderby sbTable.CMD_INSER_TIME descending
                                  select sbTable.CMD_INSER_TIME).Take(1);
            ...
            */
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = (from sbTable in con.HCMD.AsNoTracking()
    ...
    public class CmdConverter
    {
        Definition.Convert defCvt = new Definition.Convert();
        #region CMD
        public ViewerObject.VCMD GetVCMD(ACMD input)
        {
            ...
            if (input != null)
            {
                ...
                {
                    CMD_ID = input.ID?.Trim() ?? "",
                    CMD_TYPE = defCvt.GetCmdType(input.CMD_TYPE),
                    ...
                    CMD_STATUS = defCvt.GetCmdStatus(input.CMD_STATUS),
                    TRANSFER_ID = input.TRANSFER_ID?.Trim() ?? ""
                };
            }
            return output;
        }
        public List<ViewerObject.VCMD> GetVCMDs(List<ACMD> input)
        {
        ...
        public ViewerObject.VCMD GetVCMD(HCMD input)
        {
            ViewerObject.VCMD output = null;
            if (input != null)
            {
                ...
                {
                    CMD_ID = input.ID?.Trim() ?? "",
                    CMD_TYPE = defCvt.GetCmdType((E_CMD_TYPE)input.CMD_TYPE),
                    VH_ID = input.VH_ID?.Trim() ?? "",
                    CARRIER_ID = input.CARRIER_ID?.Trim() ?? "",
                    //LOT_ID = input.LOT_ID?.Trim() ?? "",
                    SOURCE = input.SOURCE?.Trim() ?? "",
                    ...
                    START_TIME = input.CMD_START_TIME,
                    END_TIME = input.CMD_END_TIME,
                    CMD_STATUS = defCvt.GetCmdStatus((E_CMD_STATUS)input.CMD_STATUS),
                    TRANSFER_ID = input.TRANSFER_ID?.Trim() ?? ""
                };
            }
            return output;
        }
        public List<ViewerObject.VCMD> GetVCMDs(List<HCMD> input)
        {
            List<ViewerObject.VCMD> output = new List<ViewerObject.VCMD>();
            if (input?.Count > 0)
            {
        ...
        public ViewerObject.REPORT.VCMD_ExportDetail GetVCMD_Detail(HCMD input)
        {
        ...
        public List<ViewerObject.REPORT.VCMD_ExportDetail> GetVCMD_Details(List<HCMD> input)
        {
        ...
        #region TRANSFER
        public ViewerObject.VTRANSFER GetVTRANSFER(VTRANSFER input)
        {
            ...
            if (input != null)
            {
                output = new ViewerObject.VTRANSFER()
                {
                    ...
                    TRANSFER_STATUS = GetTransferStatus(input.TRANSFERSTATE),
                    CMD_ID = input.EXCUTE_CMD_ID?.Trim() ?? "",
                    VH_ID = input.VH_ID?.Trim() ?? ""
                };
            }
            return output;
        }
        public List<ViewerObject.VTRANSFER> GetVTRANSFERs(List<VTRANSFER> input)
        {
        ...
        public ViewerObject.VTRANSFER_Def.TransferStatus GetTransferStatus(E_TRAN_STATUS transfer_status)
        {
            switch (transfer_status)
            {
                case E_TRAN_STATUS.Reject:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.Reject;
                case E_TRAN_STATUS.RouteChanging:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.RouteChanging;
                case E_TRAN_STATUS.Complete:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.Complete;
                case E_TRAN_STATUS.Aborted:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.Aborted;
                case E_TRAN_STATUS.Canceled:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.Canceled;
                ...
                case E_TRAN_STATUS.Initial:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.Initial;
                case E_TRAN_STATUS.PreInitial:
                    return ViewerObject.VTRANSFER_Def.TransferStatus.PreInitial;
