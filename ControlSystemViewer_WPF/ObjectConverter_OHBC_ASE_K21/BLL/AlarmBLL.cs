            ...
            if (input?.Count > 0)
            {
                foreach (var i in input)
                {
                    ...
                    if (alarmMap != null)
                    {
                        ...
                        else if (isOHCV_7(o.EQPT_ID))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHCV_7") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (isOHCV_5(o.EQPT_ID))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHCV_5") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (isNTB(o.EQPT_ID))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "NTB") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (o.EQPT_ID.Contains("_A"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "AGV") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "LINE") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
