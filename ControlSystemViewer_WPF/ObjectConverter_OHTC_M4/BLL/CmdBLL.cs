...
{ 
    public class CmdBLL : ICmdBLL
    {
        ...
        public List<ViewerObject.REPORT.VCMD_ExportDetail> GetVCMDDetail_Export(DateTime startDatetime, DateTime endDatetime)
                                                           => getHCMD_Detail_Export(startDatetime, endDatetime );
        private List<ViewerObject.REPORT.VCMD_ExportDetail> getHCMD_Detail_Export(DateTime startDatetime, DateTime endDatetime)
        {
            List<ViewerObject.REPORT.VCMD_ExportDetail> result = new List<ViewerObject.REPORT.VCMD_ExportDetail>();
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = from cmd in con.HCMD.AsNoTracking()
                                join tran in con.HTRANSFER.AsNoTracking() 
                                on new { ID = cmd.TRANSFER_ID, CARRIER_ID = cmd.CARRIER_ID, TIME = cmd.TRANSFER_CMD_INSER_TIME??DateTime.MinValue }
                                equals new { ID = tran.ID, CARRIER_ID = tran.CARRIER_ID, TIME = tran.CMD_INSER_TIME }
                                into ps
                                from tran in ps.DefaultIfEmpty()
                                ...
                                {
                                    CMD_ID = cmd.ID,
                                    CMD_TYPE = (E_CMD_TYPE)cmd.CMD_TYPE,
                                    VH_ID = cmd.VH_ID,
                                    ...
                                    START_TIME = cmd.CMD_START_TIME,
                                    END_TIME = cmd.CMD_END_TIME,
                                    CMD_STATUS = (E_CMD_STATUS)cmd.CMD_STATUS,
                                    TRANSFER_ID = cmd.TRANSFER_ID,
                                    CompeleteStatus = cmd.COMPLETE_STATUS,
                                    CARRIER_ID_ON_CRANE = tran.CARRIER_ID_ON_CRANE,
                                    Carrer_Type= cmd.CARRIER_TYPE
                                };

                    foreach (var item in query?.ToList())
                    {
                        result.Add(new ViewerObject.REPORT.VCMD_ExportDetail()
                                 {
                                     CMD_ID = item.CMD_ID?.Trim() ?? "",
                                     CMD_TYPE = defCvt.GetCmdType((E_CMD_TYPE)item.CMD_TYPE),
                                     VH_ID = item.VH_ID?.Trim() ?? "",
                                     CARRIER_ID = item.CARRIER_ID?.Trim() ?? "",
                                     //LOT_ID = input.LOT_ID?.Trim() ?? "",
                                     ...
                                     CMD_STATUS = defCvt.GetCmdStatus((E_CMD_STATUS)item.CMD_STATUS),
                                     TRANSFER_ID = item.TRANSFER_ID?.Trim() ?? "",
                                     CompeleteStatus = item.CompeleteStatus.ToString() ?? "",
                                     CARRIER_ID_ON_CRANE = item.CARRIER_ID_ON_CRANE,
                                     CARRIER_TYPE = item.Carrer_Type == null ?"": item.Carrer_Type.ToString()
                        }
                        ); ;
                    }
            ...
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                var query = from tsf in con.HTRANSFER.AsNoTracking()
                            ...
                            select new ViewerObject.VTRANSFER
                            {
                                ...
                                REJECT_REASON= tsf.REJECT_REASON,
                                COMPLETE_STATUS = tsf.COMPLETE_STATUS.ToString()
                    ...
                    var query = from sbTable in con.VHCMD_OHTC_MCS.AsNoTracking()
                                ...
                                &&
                                 (sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusUnload ||         //normal end
                                                   ...
                                                   sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIdemptyRetrival || //emptyretrival
                                                   sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIddoubleStorage || //doublestorage
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var queryNumbers = from sbTable in con.VHCMD_OHTC_MCS.AsNoTracking()
                                       ...
                                       where ((sbTable.MCS_CMD_INSER_TIME < FinishTime && sbTable.MCS_CMD_INSER_TIME >= StartTime) || (sbTable.MCS_CMD_START_TIME < FinishTime && sbTable.MCS_CMD_START_TIME >= StartTime))
                                              ...
                                              (sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusUnload ||         //normal end
                                               ...
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIddoubleStorage || //doublestorage
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusInterlockError ||  //interlock
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusLoadInterlockError ||  //interlock
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusUnloadInterlockError ||  //interlock
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusVehicleAbort ||    //vehicleabort
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusScan ||            //scan    
                                               sbTable.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIdreadFailed       //barcoderead
                                               )
                    ...
                    var query = from cmd in (
                                ...
                                select new
                                {
                                    ...
                                    EmptyRetrieval = g.Sum(i => i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIdemptyRetrival ? (Int32)1 : 0),
                                    DoubleStorage = g.Sum(i => i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusIddoubleStorage ? (Int32)1 : 0),
                                    ...
                                    #region M4 Customer Culomn
                                    LoadPIOError = g.Sum(i => (i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusLoadPioerror) ? (int)1 : 0),
                                    UnLoadPIOError = g.Sum(i => (i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusUnloadPioerror) ? (int)1 : 0),
                                    #endregion
                                    LoadingInterLock = g.Sum(i => (i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusLoadInterlockError) ? (int)1 : 0),
                                    UnLoadingInterLock = g.Sum(i => (i.OHTC_COMPLETE_STATUS == (int)CompleteStatus.CmpStatusUnloadInterlockError) ? (int)1 : 0)
                                };
                    ...
                    foreach (var item in query?.ToList())
                    {
                        ...
                        transExecRates.Add(new ViewerObject.REPORT.VTransExecRate(
                                            ...
                                            StartTime: StartTime, EndTime: FinishTime, _TransTime128: item.LeadTime128_5, _TransExecTime128: item.TransExecTime128_5
                                                ...
                                                {
                                                    LoadPIOError = item.LoadPIOError,
                                                    UnLoadPIOError=item.UnLoadPIOError
                                                }
                                          ); ;
                    }
                }
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var i in query?.ToList())
                    {
                        ViewerObject.REPORT.VRealExcuteTime oVRealExcuteTime = new ViewerObject.REPORT.VRealExcuteTime()
                        {
                            ...
                            Mean_CMDQUEUE_TIME = Math.Round(i.Mean_CMDQUEUE_TIME,2),
                            Mean_CMD_TOTAL_EXCUTION_TIME = Math.Round(i.Mean_CMD_TOTAL_EXCUTION_TIME,2),
                            Mean_MOVE_TO_SOURCE_TIME = Math.Round(i.Mean_MOVE_TO_SOURCE_TIME,2),
                            Mean_MOVE_TO_DESTN_TIME = Math.Round(i.Mean_MOVE_TO_DESTN_TIME,2)
                        };
        ...
        public ViewerObject.REPORT.VCMD_ExportDetail GetVCMD_Detail(HCMD input)
        {
            ...
            if (input != null)
            {
                output = new ViewerObject.REPORT.VCMD_ExportDetail()
                {
                    ...
                    TRANSFER_ID = input.TRANSFER_ID?.Trim() ?? "",
                    CompeleteStatus = input.COMPLETE_STATUS.ToString() ?? "",
                    CARRIER_TYPE = input.CARRIER_TYPE.ToString()
                };
            }
            return output;
        }     
