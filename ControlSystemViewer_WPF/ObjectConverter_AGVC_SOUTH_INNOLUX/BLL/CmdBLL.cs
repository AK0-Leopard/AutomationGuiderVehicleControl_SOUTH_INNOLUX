...
using ObjectConverter_AGVC_SOUTH_INNOLUX;
using ViewerObject;
...
namespace ObjectConverter_AGVC_SOUTH_INNOLUX.BLL
{
    public class CmdBLL : ICmdBLL
    {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in query?.ToList())
                    {
                        ...
                        VCMD_OHTC oVCMD_OHTC = new VCMD_OHTC()
                        {
                            ...
                            MCS_CMD_INSERTTIME = item.MCS_CMD_INSER_TIME,
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = from cmd in subquery
                                ...
                                select new
                                {
                                    VH_ID = g.Key.VH_ID,
                                    CMDCount = g.Count(),
                                    EQPTAbortErrorCount = g.Sum(i => (i.OHTC_CMPSTATUS == CompleteStatus.CmpStatusVehicleAbort) ? (Int32)1 : 0),
                                };
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = from cmd in (
                                ...
                                select new
                                {
                                    ...
                                    CompletedCount = g.Sum(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (Int32)1 : 0),
                                    InterLockCount = g.Sum(i => i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError ? (Int32)1 : 0),
                                    EQPTAbortErrorCount = g.Sum(i => i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusVehicleAbort ? (Int32)1 : 0),
                                    BarcodeReadFail = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusIdreadFailed) ? (Int32)1 : 0)),

                                    LeadTime128 = g.Sum(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_INSER_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,
                                    LeadTime128_5 = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) && i.OHTC_CMD_TYPE == 5) ? (DbFunctions.DiffSeconds(i.MCS_CMD_INSER_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,

                                    TransExecTime = g.Sum(i => (DbFunctions.DiffMinutes(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME))) ?? 0,
                                    TransExecTime128 = g.Sum(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,
                                    TransExecTime128_5 = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) && i.OHTC_CMD_TYPE == 5) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,
                                    MaxTransExecTime = (g.Max(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0),
                                    MinTransExecTime = (int?)(g.Min(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) && i.OHTC_CMD_TYPE == 5) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : null) ?? null),

                                    LoadingInterLock = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError) && ((i.COMMANDSTATE & 4) == 4) && ((i.COMMANDSTATE & 32) != 32)) ? (int)1 : 0),
                                    UnLoadingInterLock = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError) && ((i.COMMANDSTATE & 32) == 32)) ? (int)1 : 0)
                                };
                    ...
                    foreach (var item in query?.ToList())
                    {
                        ...
                        transExecRates.Add(new ViewerObject.REPORT.VTransExecRate(
                                            ...
                                            _MCSTotalCount: iMSTTotalCount, _EntryCount: item.EntryCount, _CompletedCount: item.CompletedCount, _EmptyRetrievalErrorCount: 0, _DoubleStorageErrorCount: 0, _InterLockCount: item.InterLockCount, _EQPTAbortErrorCount: item.EQPTAbortErrorCount,
                                            _BarcodeReadFailCount: item.BarcodeReadFail, _ScanCount: 0,
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in query?.ToList())
                    {
                        transExecRates.Add(new ViewerObject.REPORT.VTransExecRateByVHID(
                                            ...
                                            _MCSTotalCount: MSTTotalCount[item.VH_ID.VH_ID.ToString()], _EntryCount: item.EntryCount, _CompletedCount: item.CompletedCount, _EmptyRetrievalErrorCount: 0, _DoubleStorageErrorCount: 0, _InterLockCount: item.InterLockCount, _EQPTAbortErrorCount: item.EQPTAbortErrorCount,
                                              _BarcodeReadFailCount: item.BarcodeReadFail, _ScanCount: 0,
                                               _LoadingInterLock: item.LoadingInterLock, _UnLoadingInterLock: item.UnLoadingInterLock,
                                            _AvgLeadTime: (item.CompletedCount != 0) ? Math.Round((double)(item.LeadTime128_5 / item.CompletedCount), 2) : 0,
                                            _CycleTime: item.TransExecTime, _AvgCycleTime: (item.CompletedCount != 0) ? Math.Round((double)(item.TransExecTime128_5 / item.CompletedCount), 2) : 0, _MaxCycleTime: item.MaxTransExecTime, _MinCycleTime: item.MinTransExecTime,
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = from cmd in (from sbTable in con.VHCMD_OHTC_MCS.AsNoTracking()
                                ...
                                select new
                                {
                                    ...
                                    CycleTime128 = g.Sum(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,
                                    CycleTime128_5 = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) && i.OHTC_CMD_TYPE == 5) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0,
                                    MaxCycleTime = (g.Max(i => (i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : 0) ?? 0),
                                    MinCycleTime = (int?)(g.Min(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload || i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload) && i.OHTC_CMD_TYPE == 5) ? (DbFunctions.DiffSeconds(i.MCS_CMD_START_TIME, i.MCS_CMD_FINISH_TIME)) : null) ?? null),

                                    LoadingInterLock = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError) && ((i.MCS_COMMANDSTATE & 4) == 4) && ((i.MCS_COMMANDSTATE & 32) != 32)) ? (int)1 : 0),
                                    UnLoadingInterLock = g.Sum(i => ((i.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError) && ((i.MCS_COMMANDSTATE & 32) == 32)) ? (int)1 : 0)
                                };
                    ...
                    foreach (var item in query?.ToList())
                    {
                        transExecRateDetails.Add(new ViewerObject.REPORT.VTransExecRateDetail(
                                            ...
                                            _MCSTotalCount: MSTTotalCount[item.Year + "-" + item.Month + "-" + item.Day + "-" + item.Hour], _EntryCount: item.EntryCount, _CompletedCount: item.CompletedCount, _EmptyRetrievalErrorCount: 0, _DoubleStorageErrorCount: 0, _InterLockCount: item.InterLockCount, _EQPTAbortErrorCount: item.EQPTAbortErrorCount,
        ...
        private static int E_MCSCommandState_To_Int(E_MCSCommandState iCommandState)
        {

            if (iCommandState == E_MCSCommandState.CommandFinish) { return 224; }
            if (iCommandState == E_MCSCommandState.DoubleStorage) { return 256; }
            if (iCommandState == E_MCSCommandState.EmptyRetrieval) { return 512; }
            if (iCommandState == E_MCSCommandState.InterLockError) { return 1024; }
        ...
        public int MCS_CommandFinish = E_MCSCommandState_To_Int(E_MCSCommandState.CommandFinish);
        public int MCS_DoubleStorage = E_MCSCommandState_To_Int(E_MCSCommandState.DoubleStorage);
        public int MCS_EmptyRetrieval = E_MCSCommandState_To_Int(E_MCSCommandState.EmptyRetrieval);
        public int MCS_InterLockError = E_MCSCommandState_To_Int(E_MCSCommandState.InterLockError);
        ...
        public ViewerObject.VCMD GetVCMD(HCMD_OHTC input)
        {
            ...
            if (input != null)
            {
                output = new ViewerObject.VCMD()
                {
                    ...
                    TRANSFER_ID = input.CMD_ID_MCS?.Trim(),
                    CMD_STATUS = defCvt.GetCmdStatus(input.CMD_STAUS),
                    CompeleteStatus = input.CMD_TPYE == E_CMD_TYPE.LoadUnload ? "CmpStatusLoadunload" : ""
        ...
        public ViewerObject.VTRANSFER GetVTRANSFER(VACMD_MCS input)
        {
            ...
            if (input != null)
            {
                output = new ViewerObject.VTRANSFER()
                {
                    ...
                    VH_ID = input.VH_ID?.Trim() ?? "",
                    COMMANDSTATE = (VTRANSFER_Def.CommandState)input.COMMANDSTATE
                };
            }
            return output;
        }
