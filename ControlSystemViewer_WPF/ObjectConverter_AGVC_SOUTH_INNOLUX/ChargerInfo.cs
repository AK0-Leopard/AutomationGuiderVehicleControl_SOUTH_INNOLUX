...
namespace ObjectConverter_AGVC_SOUTH_INNOLUX
{
    ...
    {
        private readonly string ns = "ObjectConverter_AGVC_SOUTH_INNOLUX" + ".ChargerInfo";
        ...
        public bool GetCouplerInfos(string input, out List<CouplerInfo> output, out string result)
        {
            ...
            try
            {
                ...
                {
                    result = $"{ns}.{ms} - JsonConvert.DeserializeObject<List<CouplerInfo>> result = null or empty, input: {input}";
                    return false;
                }
            }
            catch (Exception ex)
        ...
        public bool SetCharger(ref ViewerObject.Charger obj, CouplerInfo coupler, AUNIT unit, out string result)
        {
            ...
            try
            {
                ...
                if (id != null)
                {
                    ...
                    oCoupler = new AGVC_SOUTH_INNOLUX_Coupler(id.Value, "Coupler" + (id).ToString(), obj.ChargerID);
                    oCoupler.AddressID = coupler.AddressID;
                    _doing = "Set Coupler Status";
                    switch (id)
                    {
                        case 1:
                            oCoupler.Status = unit?.coupler1Status_NORTH_INNOLUX == null ? ViewerObject.Coupler_Def.CouplerStatus.None :
                             new Definition.Convert().GetCouplerStatus(unit.coupler1Status_NORTH_INNOLUX);
                            oCoupler.ShowStatus = CouplerStatusConvert(unit.coupler1Status_NORTH_INNOLUX);
                        ...
                        case 2:
                            oCoupler.Status = unit?.coupler2Status_NORTH_INNOLUX == null ? ViewerObject.Coupler_Def.CouplerStatus.None :
                             new Definition.Convert().GetCouplerStatus(unit.coupler2Status_NORTH_INNOLUX);
                            oCoupler.ShowStatus = CouplerStatusConvert(unit.coupler2Status_NORTH_INNOLUX);
                        ...
                        case 3:
                            oCoupler.Status = unit?.coupler3Status_NORTH_INNOLUX == null ? ViewerObject.Coupler_Def.CouplerStatus.None :
                             new Definition.Convert().GetCouplerStatus(unit.coupler3Status_NORTH_INNOLUX);
                            oCoupler.ShowStatus = CouplerStatusConvert(unit.coupler3Status_NORTH_INNOLUX);
        ...
        private string CouplerStatusConvert(com.mirle.ibg3k0.sc.App.SCAppConstants.CouplerStatus_NORTH_INNOLUX status)
        {
            if (Enum.IsDefined(typeof(com.mirle.ibg3k0.sc.App.SCAppConstants.CouplerStatus_NORTH_INNOLUX), status))
            {
                switch (status)
                {
