...
namespace ObjectConverter_OHTC_AT_S_MALASYIA.BLL
{
    public class AlarmBLL : IAlarmBLL
    {
        ...
        private List<ALARM_PROCESS_DETAIL> getAlarmsByConditions(DateTime startDatetime, DateTime endDatetime, string eqptID = null, string alarmCode = null, E_ALARM_LVL Alarm_Level = E_ALARM_LVL.None, bool cleartimenotnull = false)
        {
            List<ALARM_PROCESS_DETAIL> alarms;
            using (var dbCon = new OHTC_AT_S_MALASYIAEntities())
            {
                if (!string.IsNullOrWhiteSpace(connectionString))
                    dbCon.Database.Connection.ConnectionString = connectionString;
                alarms = (from a in dbCon.ALARM_PROCESS_DETAIL
                              where
                                a.RPT_DATE_TIME >= startDatetime && 
                                a.RPT_DATE_TIME <= endDatetime && 
                                (Alarm_Level == E_ALARM_LVL.None || a.ALAM_LVL == (int)Alarm_Level) && 
                                ((cleartimenotnull == false) || (cleartimenotnull == true && a.CLEAR_DATE_TIME != null))
                              select a).ToList();
                alarms = alarms.Where(t =>
                {
                    bool b = true;
                    if (eqptID != null)
                    {
        ...
        public List<ViewerObject.REPORT.VErrorCMD> LoadAlarmsByErrorMCSCmdTime(DateTime startDatetime, DateTime endDatetime, string Alarm_Level = null, int TimeIntervalAdd = 0, List<ViewerObject.AlarmMap> alarmMap = null, List<ViewerObject.AlarmModule> alarmModule = null)
                                        => loadalarmsbyerrormcscmdtime(startDatetime, endDatetime, CheckAlarmLevel(Alarm_Level), TimeIntervalAdd, alarmMap, alarmModule);
        private List<ViewerObject.REPORT.VErrorCMD> loadalarmsbyerrormcscmdtime(DateTime startDatetime, DateTime endDatetime, E_ALARM_LVL Alarm_Level = E_ALARM_LVL.None, int TimeIntervalAdd = 0, List<ViewerObject.AlarmMap> alarmMap = null, List<ViewerObject.AlarmModule> alarmModule = null)
        {
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                        ...
                        oVErrorCMD.CARRIER_TYPE = item.CARRIER_TYPE!=null? item.CARRIER_TYPE.ToString():"";
        ...
        public List<ViewerObject.REPORT.VTimeOutCMD> LoadAlarmsByTimeOutCycleTime_MCSCmd(DateTime startDatetime, DateTime endDatetime, int OverTime_Min, string Alarm_Level = null, int TimeIntervalAdd = 0, List<ViewerObject.AlarmMap> alarmMap = null, List<ViewerObject.AlarmModule> alarmModule = null)
                                          => loadalarmsbytimeoutcycletime_mcscmd(startDatetime, endDatetime, OverTime_Min, CheckAlarmLevel(Alarm_Level), TimeIntervalAdd,alarmMap,alarmModule);
        private List<ViewerObject.REPORT.VTimeOutCMD> loadalarmsbytimeoutcycletime_mcscmd(DateTime startDatetime, DateTime endDatetime, int OverTime_Min, E_ALARM_LVL Alarm_Level = E_ALARM_LVL.None, int TimeIntervalAdd = 0, List<ViewerObject.AlarmMap> alarmMap = null, List<ViewerObject.AlarmModule> alarmModule = null)
        {
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                        ...
                        oVErrorCMD.OHTC_CMD_STATUS = DefineInt_To_EOHTC_CMD_STATUS(item.COMPLETE_STATUS);
        ...
        private Define.E_OHTC_CMD_STATUS DefineInt_To_EOHTC_CMD_STATUS(CompleteStatus? input)
        {
            try
            {
                switch (input)
        ...
        public bool AlarmUpdate(ControlRequest request, out string result)
        {
            ...
            DateTime dt_rpt_time = DateTime.FromBinary(request.RPTDATETIME);
            int updateResult = 0;
            using (var dbCon = new OHTC_AT_S_MALASYIAEntities())
            {
                if (!string.IsNullOrWhiteSpace(connectionString))
                    dbCon.Database.Connection.ConnectionString = connectionString;
                var detail = (from a in dbCon.ALARM_PROCESS
                          where
                              a.EQPT_ID == request.EQPTID &&
                              a.RPT_DATE_TIME == dt_rpt_time &&
                              a.ALAM_CODE == request.ALARMCODE
                          select a).FirstOrDefault();

                if (detail == null)
                {
                    detail = new ALARM_PROCESS();

                    detail.ALAM_CODE = request.ALARMCODE;
                    detail.EQPT_ID = request.EQPTID;
                    detail.RPT_DATE_TIME = dt_rpt_time;

                    detail.CLASS = (int)request.ALARMCLASSIFICATION;
                    detail.REMARK = request.ALARMREMARK;
                    detail.IMPORTANCE_LVL = Convert.ToInt32(request.IMPORTANCELEVEL);
                    detail.ALARM_MODULE = Convert.ToInt32(request.ALARMMODULE);
                    dbCon.ALARM_PROCESS.Add(detail);
                }
                else
                {
                    detail.CLASS = (int)request.ALARMCLASSIFICATION;
                ...
                updateResult = dbCon.SaveChanges();
    ...
    public class AlarmConverter
    {
        ...
        public List<ViewerObject.VALARM> GetVALARMs(List<ALARM> input, List<ViewerObject.AlarmModule> alarmModule = null, List<ViewerObject.AlarmMap> alarmMap = null)
        {
            List<ViewerObject.VALARM> output = new List<ViewerObject.VALARM>();
            if (input?.Count > 0)
            {
                foreach (var i in input)
                {
                    ...
                    if (alarmMap != null)
                    {
                        ...
                        //若傳入非空則做英翻中
                        if (o.EQPT_ID.Trim() == "OHTC")
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHTC_NODE") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (o.EQPT_ID.Contains("OHT"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHT_NODE") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (o.EQPT_ID.Contains("HID"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "HID_NODE") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (o.EQPT_ID.Contains("MTL"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "MTL_NODE") && (alarmdesc.alarmID == o.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        //else
                        //    temp = (from alarmdesc in alarmMap
                    ...
                    if (o != null)
                    {
                        trySetAlarmModuleInfo(o, alarmModule);//M4不用alarm Module, 目前只做語系轉換
                        output.Add(o);
                    }
                }
            }
        ...
        public List<ViewerObject.VALARM> GetVALARMs(List<ALARM_PROCESS_DETAIL> input, List<ViewerObject.AlarmModule> alarmModule = null, List<ViewerObject.AlarmMap> alarmMap = null)
        {
            List<ViewerObject.VALARM> output = new List<ViewerObject.VALARM>();
            string _doing = "";
            if (input?.Count > 0)
            {
                foreach (var i in input)
                {
                    ...
                    try
                    {
                        ...
                        valarm.RPT_DATE_TIME = i.RPT_DATE_TIME.ToString("yyyy/MM/dd HH:mm:ss");
                        valarm._RPT_DATE_TIME = i.RPT_DATE_TIME;
                        _doing = $"Set EQPT_ID";
                        valarm.EQPT_ID = i.EQPT_ID?.Trim() ?? "";
                        _doing = $"Set ALARM_LVL";
                        valarm.ALARM_LVL = (AlarmLvl)i.ALAM_LVL;
                        _doing = $"Set ALARM_CODE";
                        valarm.ALARM_CODE = i.ALAM_CODE?.Trim() ?? "";
                        _doing = $"Set ALARM_DESC";
                        valarm.ALARM_DESC = i.ALAM_DESC?.Trim() ?? "";
                        _doing = $"Set IS_CLEARED";
                        valarm.IS_CLEARED = (ErrorStatus)i.ALAM_STAT == ErrorStatus.ErrReset;
                        _doing = $"Set CLEAR_DATE_TIME";
                        valarm.CLEAR_DATE_TIME = i.CLEAR_DATE_TIME?.ToString("yyyy/MM/dd HH:mm:ss") ?? "";
                        _doing = $"Set VH_INSTALL_FLAG";

                        valarm.ALARM_REMARK = i.REMARK;
                        valarm.ALARM_MODULE = i.ALARM_MODULE == null ? 0 : (int)i.ALARM_MODULE;
                        valarm.IMPORTANCE_LEVEL = i.IMPORTANCE_LVL == null ? AlarmImportanceLevel.Low : (AlarmImportanceLevel)i.IMPORTANCE_LVL;
                        valarm._ALARM_CLASSIFICATION = i.CLASS == null ? 0 : (int)i.CLASS;
                        valarm.CMD_ID = i.CMD_ID_1;
                        if (i.VH_INSTALL_FLAG == null)
                        {
                            //Default Null
                        }
                        else
                        {
                            if (i.VH_INSTALL_FLAG == true)
                            {
                                valarm.VH_INSTALL_FLAG = ViewerObject.VALARM_Def.VH_INSTALL_FLAG.Install.ToString();
                            }
                            else
                            {
                                valarm.VH_INSTALL_FLAG = ViewerObject.VALARM_Def.VH_INSTALL_FLAG.Remove.ToString();
                            }
                        }
                        _doing = $"Set RPT_DATE_TIME";
                        valarm._RPT_DATE_TIME = i.RPT_DATE_TIME;
                    ...
                    if (alarmMap != null)
                    {
                        ViewerObject.AlarmMap temp = null;
                        //若傳入非空則做英翻中
                        if (valarm.EQPT_ID.Trim() == "OHTC")
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHTC_NODE") && (alarmdesc.alarmID == valarm.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (valarm.EQPT_ID.Contains("OHT"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "OHT_NODE") && (alarmdesc.alarmID == valarm.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (valarm.EQPT_ID.Contains("HID"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "HID_NODE") && (alarmdesc.alarmID == valarm.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        else if (valarm.EQPT_ID.Contains("MTL"))
                            temp = (from alarmdesc in alarmMap
                                    where (alarmdesc.objectID == "MTL_NODE") && (alarmdesc.alarmID == valarm.ALARM_CODE)
                                    select alarmdesc).FirstOrDefault();
                        //else
                        //    temp = (from alarmdesc in alarmMap
                        //            where (alarmdesc.objectID == "TRACK") && (alarmdesc.alarmID == o.ALARM_CODE)
                        //            select alarmdesc).FirstOrDefault();
                        if (temp != null && temp.alarmDesc_TW != "")
                            valarm.ALARM_DESC = temp.alarmDesc_TW;
                    }
                    if (valarm != null)
                    {
                        trySetAlarmModuleInfo(valarm, alarmModule);//M4不用alarm Module, 目前只做語系轉換
                        output.Add(valarm);
                    }
                }
            }
