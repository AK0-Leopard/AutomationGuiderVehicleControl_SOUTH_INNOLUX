...
namespace ObjectConverter_OHTC_AT_S
{
    ...
    {
        private readonly string ns = "ObjectConverter_OHTC_AT_S" + ".APortStation";
        ...
        public bool SetVPORTSTATION(APORTSTATION aport, out ViewerObject.VPORTSTATION port, out string result)
        {
            ...
            try
            {
                ...
                port = new ViewerObject.VPORTSTATION(aport.PORT_ID, aport.ADR_ID, aport.stageCount);
                _doing = "IS_IN_SERVICE";
                port.IS_MONITORING = port.PORT_ID.StartsWith("OHT");
                _doing = "IS_IN_SERVICE";
                port.IS_IN_SERVICE = aport.PORT_SERVICE_STATUS == PortStationServiceStatus.InService;
