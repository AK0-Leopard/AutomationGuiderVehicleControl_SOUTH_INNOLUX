                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var joinquery = from sbOHTC in con.HTRANSFER.AsNoTracking()
                                    ...
                                    select new
                                    {
                                        ...
                                        MCS_CMD_FINISH_TIME = VHCMD_MCSi.CMD_END_TIME,
                                        OHTC_CMD_STATUS = VHCMD_MCSi.CMD_STATUS,
                                        VH_ID = VHCMD_MCSi.VH_ID,
                                        CMD_ID_MCS = VHCMD_MCSi.ID,
                                        CARRIER_ID = sbOHTC.CARRIER_ID,
                                        HOSTSOURCE = VHCMD_MCSi.SOURCE,
                                        HOSTDESTINATION = VHCMD_MCSi.DESTINATION,
                                        MCS_CMD_INSER_TIME = VHCMD_MCSi.CMD_INSER_TIME
                                    };
                    ...
                    var CMDquery = from sbTable in joinquery
                                   where (sbTable.MCS_CMD_FINISH_TIME < endDatetime && sbTable.MCS_CMD_START_TIME >= startDatetime)
                                         &&
                                         (sbTable.OHTC_CMD_STATUS == E_CMD_STATUS.AbnormalEndByOHT) &&
                                         sbTable.VH_ID != null &&
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                        ...
                        oVErrorCMD.OHTC_CMD_STATUS = ConvertCMD_STATUStoRPT(item.OHTC_CMD_STATUS);
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                        ...
                        oVTimeOutCMD.OHTC_CMD_STATUS = ConvertCMD_STATUStoRPT(item.OHTC_CMD_STATUS);
