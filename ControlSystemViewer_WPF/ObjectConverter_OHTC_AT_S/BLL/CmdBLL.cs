...
namespace ObjectConverter_OHTC_AT_S.BLL
{ 
    public class CmdBLL : ICmdBLL
    {
        ...
        private List<com.mirle.ibg3k0.sc.VTRANSFER> loadUnfinishedTransfers()
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                if (!string.IsNullOrWhiteSpace(connectionString))
                ...
                var query = from cmd in con.VTRANSFER.AsNoTracking()
                            ...
                            select cmd;
                return query?.ToList() ?? new List<com.mirle.ibg3k0.sc.VTRANSFER>();
            }
        }

        public ViewerObject.VTRANSFER GetTransferByID(string id) => cmdConverter.GetVTRANSFER(getTransferByID(id));
        private com.mirle.ibg3k0.sc.VTRANSFER getTransferByID(string id)
        {
            if (string.IsNullOrWhiteSpace(id)) return null;
            id = id.Trim();
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var query = from sbTable in joinquery.AsNoTracking()
                                ...
                                &&
                                 (sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusUnload ||         //normal end
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusLoadunload ||
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusInterlockError ||  //interlock
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusVehicleAbort ||    //vehicleabort 
                                                   sbTable.OHTC_COMPLETE_STATUS == CompleteStatus.CmpStatusIdreadFailed       //barcoderead
                                                   ) &&
    ...
    public class CmdConverter
    {
        ...
        #region TRANSFER
        public ViewerObject.VTRANSFER GetVTRANSFER(com.mirle.ibg3k0.sc.VTRANSFER input)
        {
        ...
        public List<ViewerObject.VTRANSFER> GetVTRANSFERs(List<com.mirle.ibg3k0.sc.VTRANSFER> input)
        {
