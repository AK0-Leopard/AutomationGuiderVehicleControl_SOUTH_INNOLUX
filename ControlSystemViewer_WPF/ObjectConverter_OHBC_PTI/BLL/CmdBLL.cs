        ...
        private List<VACMD_MCS> loadUnfinishedTransfers()
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                var query = from cmd in con.VACMD_MCS.AsNoTracking()
                            where (int)cmd.TRANSFERSTATE >= (int)E_TRAN_STATUS.Queue && (int)cmd.TRANSFERSTATE <= (int)E_TRAN_STATUS.Aborting
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in query?.ToList())
                    {
                        ...
                        VCMD_OHTC oVCMD_OHTC = new VCMD_OHTC()
                        {
                            ...
                            MCS_CMD_INSERTTIME =Convert.ToDateTime(item.MCS_CMD_INSER_TIME),
