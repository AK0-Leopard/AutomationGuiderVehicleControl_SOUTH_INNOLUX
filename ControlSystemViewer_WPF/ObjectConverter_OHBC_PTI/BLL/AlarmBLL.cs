...
namespace ObjectConverter_OHBC_PTI.BLL
{
    public class AlarmBLL : IAlarmBLL
    {
        ...
        public List<ViewerObject.VALARM> GetAlarmsByConditions(DateTime startDatetime, DateTime endDatetime, string eqptID = null, string alarmCode = null, string Alarm_Level = null, bool cleartimenotnull = false, List<ViewerObject.AlarmMap> alarmMap = null, List<ViewerObject.AlarmModule> alarmModule = null) 
                                         => alarmConverter.GetVALARMs(getAlarmsByConditions(startDatetime, endDatetime, eqptID, alarmCode, Alarm_Level));
        private List<ALARM> getAlarmsByConditions(DateTime startDatetime, DateTime endDatetime, string eqptID = null, string alarmCode = null, string Alarm_Level = null, bool cleartimenotnull = false, List<ViewerObject.AlarmMap> alarmMap = null)
        {
            List<ALARM> alarms = new List<ALARM>();
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                var list = query?.ToList().Where(t =>
                {
                    ...
                    DateTime rptDateTime = DateTime.ParseExact(t.RPT_DATE_TIME.Substring(0, 17), "yyyyMMddHHmmssfff", CultureInfo.InvariantCulture);
                    return rptDateTime >= startDatetime && rptDateTime <= endDatetime && CheckAlarmLevel(t.ALAM_LVL, Alarm_Level) && ((cleartimenotnull == false) || (cleartimenotnull == true && t.END_TIME != null));
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    var Alarmquery = from cmd in con.ALARM.AsNoTracking()
                                     ...
                                     (Convert.ToDateTime(cmd.RPT_DATE_TIME) < endDatetime && Convert.ToDateTime(cmd.RPT_DATE_TIME) >= startDatetime) &&
                                     cmd.END_TIME != null
                                     select cmd;
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                        ...
                        !(((oVErrorCMD.MCS_START_TIME < Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVErrorCMD.MCS_START_TIME < Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd)) && (oVErrorCMD.MCS_FINISH_TIME < Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVErrorCMD.MCS_FINISH_TIME < Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd))) ||
                        ((oVErrorCMD.MCS_START_TIME > Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVErrorCMD.MCS_START_TIME > Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd)) && (oVErrorCMD.MCS_FINISH_TIME > Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVErrorCMD.MCS_FINISH_TIME > Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd))))
                        )
                        ).ToList());
            ...
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    foreach (var item in CMDquery?.ToList())
                    {
                       ...
                       !(((oVTimeOutCMD.MCS_START_TIME < Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVTimeOutCMD.MCS_START_TIME < Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd)) && (oVTimeOutCMD.MCS_FINISH_TIME < Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVTimeOutCMD.MCS_FINISH_TIME < Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd))) ||
                        ((oVTimeOutCMD.MCS_START_TIME > Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVTimeOutCMD.MCS_START_TIME > Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd)) && (oVTimeOutCMD.MCS_FINISH_TIME > Convert.ToDateTime(info.RPT_DATE_TIME).AddMinutes(0 - TimeIntervalAdd) && oVTimeOutCMD.MCS_FINISH_TIME > Convert.ToDateTime(info.END_TIME).AddMinutes(TimeIntervalAdd))))
                        ).ToList());
        ...
        {
            //PTI因為還有Alarm的cloumnTime為String的問題，先隱藏
            ...
            */
            ...
            try
            {
                ...
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    StartTime = query == "" ? Convert.ToDateTime(query) : DateTime.MinValue;
                    ...
                    EndTime = query1==""? Convert.ToDateTime(query1):DateTime.MaxValue;
    ...
    public class AlarmConverter
    {
        ...
        public ALARM GetALARM(ViewerObject.VALARM input)
        {
            ...
            if (input != null)
            {
                output = new ALARM()
                {
                    RPT_DATE_TIME = input.RPT_DATE_TIME,
                ...
                if (!string.IsNullOrWhiteSpace(input.CLEAR_DATE_TIME))
                    output.END_TIME = input.CLEAR_DATE_TIME;
            }
            return output;
        }
